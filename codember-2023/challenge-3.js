let passwords = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
console.log(findInvalidPassword(passwords));

function findInvalidPassword(passwords) {
  let invalidCount = 0;
  let invalidPassword;

  passwords.forEach((passwordData) => {
    const { policy, password } = passwordData;
    const { min, max, letter } = policy;
    const charCount = (password.match(new RegExp(letter, "g")) || []).length;
