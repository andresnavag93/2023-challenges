export default function canCarry(capacity, trip) {
  ...
  for (let i = 0; i < trip.length; i++) {
    if (visited.hasOwnProperty(trip[i][1])) {
      visited[trip[i][1]][0] = trip[i][0];
    } else {
      visited[trip[i][1]] = [trip[i][0], 0];
    }

    if (visited.hasOwnProperty(trip[i][2])) {
      visited[trip[i][2]][1] = trip[i][0];
    } else {
      visited[trip[i][2]] = [0, trip[i][0]];
    }

    max = Math.max(max, trip[i][1], trip[i][2]);
  }
  for (let i = 0; i <= max; i++) {
